...
# Função para calcular a indenização e criar o arquivo Excel
def calcular_indenizacao():
    # Leitura dos arquivos
    arquivo_debitos_judiciais = filedialog.askopenfilename(title="Selecione o arquivo de Débitos Judiciais")
    arquivo_supplier_line_items = filedialog.askopenfilename(title="Selecione o arquivo de Supplier Line Items")

    df = pd.read_excel(arquivo_debitos_judiciais)
    compensados = pd.read_excel(arquivo_supplier_line_items)

    # Restante do código que realiza o cálculo e formatação
    compensados = compensados.loc[compensados['Tipo lçto.contábil']=='ZP']

    data_referencia = datetime(2012, 1, 1)
    ...
    with pd.ExcelWriter('files/TabelaFinal.xlsx', engine='xlsxwriter') as excel_writer:
        ...
        messagebox.showinfo("Processamento Iniciado", "O cálculo de indenização está em andamento. Isso pode levar algum tempo.")
        ...
        # Aplicar o formato à coluna 'FATOR'
...
root = tk.Tk()
root.title("Calculadora de Indenização")

# Botão para calcular indenização
calcular_button = tk.Button(root, text="Calcular Indenização", command=calcular_indenizacao)
